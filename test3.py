import cv2
import numpy as np
import sqlite3
import pytesseract

# Ensure pytesseract is properly set up
pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'

# Function to detect license plate
def detect_license_plate(image_path):
    image = cv2.imread(image_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    filtered = cv2.bilateralFilter(gray, 11, 17, 17)
    edged = cv2.Canny(filtered, 30, 200)
    contours, _ = cv2.findContours(edged.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)[:10]

    license_plate = None
    license_plate_contour = None

    for contour in contours:
        perimeter = cv2.arcLength(contour, True)
        approx = cv2.approxPolyDP(contour, 0.018 * perimeter, True)
        if len(approx) == 4:
            license_plate_contour = approx
            x, y, w, h = cv2.boundingRect(contour)
            aspect_ratio = w / float(h)
            if 2.5 <= aspect_ratio <= 5.5:
                license_plate = gray[y:y+h, x:x+w]
                break

    if license_plate is not None:
        _, license_plate = cv2.threshold(license_plate, 64, 255, cv2.THRESH_BINARY_INV)
        kernel = np.ones((3,3), np.uint8)
        license_plate = cv2.morphologyEx(license_plate, cv2.MORPH_OPEN, kernel)

    return image, license_plate, license_plate_contour

# Function for OCR text extraction with error handling
def extract_text_from_plate(license_plate_image):
    try:
        return pytesseract.image_to_string(license_plate_image, config='--psm 8').strip()
    except pytesseract.TesseractError as e:
        print(f"Error during OCR: {e}")
        return ""

# Function to add license plate info to the database
def add_license_plate_to_database(owner_name, license_plate, apartment_info):
    conn = sqlite3.connect('society_vehicles.db')
    cursor = conn.cursor()

    # Create table if it doesn't exist, but only once
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS vehicle_owners (
            owner_name TEXT,
            license_plate TEXT UNIQUE,
            apartment_info TEXT
        )
    ''')

    try:
        cursor.execute("INSERT INTO vehicle_owners (owner_name, license_plate, apartment_info) VALUES (?, ?, ?)",
                       (owner_name, license_plate, apartment_info))
        conn.commit()
        print("License plate added to the database successfully.")
    except sqlite3.IntegrityError:
        print("This license plate already exists in the database.")
    conn.close()

# Function to verify license plate from the database
def verify_license_plate(plate_text):
    conn = sqlite3.connect('society_vehicles.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM vehicle_owners WHERE license_plate = ?", (plate_text,))
    result = cursor.fetchone()
    conn.close()
    if result:
        return f"Authorized Vehicle. Owner: {result[0]}, Apartment: {result[2]}"
    else:
        return "Unauthorized Vehicle"

# Main script logic
if __name__ == "__main__":
    image_path = 'E:/Image processing VLPR/Skoda-Superb-BW-India.jpg'  # Replace with actual image path
    original_image, detected_plate, plate_contour = detect_license_plate(image_path)

    if detected_plate is not None:
        extracted_text = extract_text_from_plate(detected_plate)
        print(f"Extracted License Plate Text: {extracted_text}")

        # Adding a new license plate to the database
        owner_name = "John Doe"
        apartment_info = "Apt 101"
        add_license_plate_to_database(owner_name, extracted_text, apartment_info)

        # Verifying the license plate against the database
        verification_result = verify_license_plate(extracted_text)
        print(verification_result)
    else:
        print("No license plate detected.")
